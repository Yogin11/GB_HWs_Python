...
# def ioFile (mode, filenum='', result=''):                   # чтение или запись в файл
    # with open ("file"+str(filenum)+".txt", mode) as f:
        ...
        # if mode == "w":
            # f.write(result)
#    
# N = int(input("Введите количество многочленов, которые нужно сформировать: "))
# N = 2
# limit = 101
# for l in range(1,N+1): 
    # while True:
        # try:
            # k = int(input(f"Введите степень {l} многочлена  больше 1:  "))
            # if k <=1:
                # print("ошибка ввода, попробуйте еще раз")
            # else:
                # slovar = {}
                # endit = random.randint(0,1)*(-1)
                # for i in range(k,endit,-1):  # формируем коэффициенты и степени членов в словаре
                    # slovar[i] = random.randint(0,limit)
                    # if i == 0:
                        # slovar[0] = random.randint(1,limit)
# 
                # final = createMnogochlen(koefistepenSlov(k,randlimit))
                # print (f"Многочлен: {l} : {final} ")    
                # ioFile("w",l,final)
                # break
        # except:
            # print("Некорректный ввод. Попробуйте еще раз")
...
# print (f"Итоговый многочлен: {result} ")
    #    
